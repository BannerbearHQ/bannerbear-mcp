 ...
 *   - Modifications (images/videos/batches) — one flat attribute bag, since a
...
const videoMods = at(
  body("/videos"),
  "properties.modifications.properties.scenes.items.properties.objects.items"
);
const batchMods = at(
  body("/batches"),
...
if (
  fingerprint(imageMods) !== fingerprint(videoMods) ||
  fingerprint(imageMods) !== fingerprint(batchMods)
) {
  console.error(
    "FATAL: modification schemas diverged between images/videos/batches.\n" +
      "The single canonical definition is no longer valid — inspect the spec."
  );
  process.exit(1);
}

// Scenes live under config.scenes, mirroring image templates' config.objects.
const sceneItems = at(body("/video_templates"), "properties.config.properties.scenes.items");
if (!sceneItems?.properties?.config) {
  console.error("FATAL: video template scenes are no longer at config.scenes[].config.");
  process.exit(1);
}
const keyframeSchema = at(sceneItems, "properties.config.properties.keyframes.additionalProperties.items");

const imageFormats = body("/images").properties.formats.items.enum;
const frameRates = body("/video_templates").properties.frame_rate?.enum ?? [24, 30, 60];
...
const kfProps = keyframeSchema.properties;
 ...
 * Verified identical across /images, /videos and /batches at codegen time.
...
/** A single animation keyframe (video only). */
export const keyframeShape = {
${shapeFrom(kfProps, Object.keys(kfProps))}
} as const;

export const Keyframe = z.object(keyframeShape);

export const IMAGE_FORMATS = ${JSON.stringify(imageFormats)} as const;
export const FRAME_RATES = ${JSON.stringify(frameRates)} as const;
export const EASINGS = ${JSON.stringify(kfProps.easing?.enum ?? [])} as const;
...
console.log(
  `generated src/generated/schemas.ts\n` +
    ...
    `${Object.keys(kfProps).length} keyframe attributes, ${kfProps.easing?.enum?.length ?? 0} easings`
);
